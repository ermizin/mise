...
function PlanBuilder({ initialPlan, initialStep = 0, initialBatchId, repeat = false, onClose, onSaved, persistPlan }: { initialPlan: ActivePlan | null; initialStep?: number; initialBatchId?: string; repeat?: boolean; onClose: () => void; onSaved: (plan: ActivePlan, destination: Tab) => void; persistPlan: (plan: ActivePlan) => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    for (const batch of batches) for (const slot of mealSlots) { const key = selectionKey(batch, slot); const recipe = recipesById[selections[key]]; if (recipe && recipe.slot === slot && recipe.tags.includes(menuStyle) && (recipe.storageDays >= batch.days || recipe.freezable)) valid[key] = recipe.id; }
    if (Object.keys(valid).length === Object.keys(selections).length) return;
    setDroppedCount(Object.keys(selections).length - Object.keys(valid).length);
    setSelections(valid);
    const firstMissing = positions.findIndex((position) => !valid[selectionKey(position.batch, position.slot)]);
    if (firstMissing >= 0) setChoiceIndex(firstMissing);
  }, [batches, mealSlots, menuStyle]); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(() => {
  ...
  function next() { if (!stepIsValid()) return; if (step === 3 && unassignedSlots.length) { setMealSlots((current) => current.filter((slot) => !unassignedSlots.includes(slot))); } if (step === 4 && !Object.keys(selections).length) setChoiceIndex(0); setStep((value) => Math.min(6, value + 1)); }
  function choose(recipeId: string) { if (!currentPosition) return; setDroppedCount(0); const key = selectionKey(currentPosition.batch, currentPosition.slot); const updated = { ...selections, [key]: recipeId }; setSelections(updated); const nextMissing = positions.findIndex((position, index) => index > choiceIndex && !updated[selectionKey(position.batch, position.slot)]); if (nextMissing >= 0) setChoiceIndex(nextMissing); }
  ...
  return <main className="app-shell builder-shell">
    ...
    <section className="builder-content">
      ...
      {droppedCount > 0 && <p className="inline-note" role="status">{droppedCount === 1 ? "Одно блюдо не подходит под новые настройки — выберите его заново на шаге «Выбор меню»." : `${droppedCount} блюда не подходят под новые настройки — выберите их заново на шаге «Выбор меню».`}</p>}
