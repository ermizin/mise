...
type RecipeStorage = { refrigerator: string; freezer: string; thaw: string; freezerDays?: number; freezeParts: string };
...
type Person = { id: string; name: string; daily: Macros; macroPreset?: MacroPreset; mealsPerDay: number; includedSlots: MealSlot[] };
...
recipes.push(
  ...
  r("src-chicken-nuggets", "snack1", "Куриные наггетсы с бататом без панировки", "🍗", 60, { kcal: 178, protein: 20.4, fat: 4.8, carbs: 13.8 }, 180, 115, ["protein", "budget", "paleo"], [
  ...
  ], 14, false, { provenance: mealPrepManualParsed("Cinnamon Raisin Granola", "cinnamon-raisin-granola", "https://mealprepmanual.com/wp-content/uploads/2023/08/Cinnamon-Raisin-Granola.jpg", "Домашняя гранола с корицей и изюмом", "Парсер ошибочно отнёс гранолу к обедам; в Mise это второй перекус. Кленовый сироп можно заменить мёдом."), storage: { refrigerator: "В сухой герметичной банке при комнатной температуре — ориентировочно до 14 суток; не убирать тёплой." }, flex: { protein: [1, 1], fat: [0.7, 1.3], carbs: [0.7, 1.3] }, effort: { knifeActions: 0, cookware: 3, activeActions: 7, activeMinutes: 7, level: "high" } }),
  ...
  ], 4, true, { provenance: mealPrepManualParsed("Peanut Turkey Stir Fry", "peanut-turkey-stir-fry", "https://mealprepmanual.com/wp-content/uploads/2024/04/Peanut-Turkey-Stir-Fry.jpg", "Индейка с замороженными овощами и арахисовым соусом", "Chinese cooking wine заменено рисовым или обычным мягким уксусом; острота снижена."), localization: { fit: "familiar", availability: "common", note: "Низкая сложность за счёт замороженных овощей и одной сковороды." }, storage: { freezerDays: 45 }, effort: { knifeActions: 0, cookware: 3, activeActions: 8, activeMinutes: 10, level: "high" } }),
...
function shareFor(person: Person, slot: MealSlot) { if (!person.includedSlots.includes(slot)) return 0; const templates: Record<number, Record<MealSlot, number>> = { 1: singleMealShares, 2: { breakfast: 0.45, lunch: 0.45, dinner: 0.55, snack1: 0.35, snack2: 0.35 }, 3: { breakfast: 0.25, lunch: 0.35, dinner: 0.4, snack1: 0.2, snack2: 0.2 }, 4: { breakfast: 0.25, lunch: 0.3, dinner: 0.35, snack1: 0.1, snack2: 0.1 }, 5: { breakfast: 0.2, lunch: 0.3, dinner: 0.3, snack1: 0.1, snack2: 0.1 } }; const weights = templates[person.mealsPerDay] ?? templates[5]; const selected = [...new Set(person.includedSlots)]; if (selected.length === 1) return singleMealShares[slot]; if (selected.length >= person.mealsPerDay) { const total = selected.reduce((sum, item) => sum + weights[item], 0); return total > 0 ? weights[slot] / total : 1 / selected.length; } return weights[slot]; }
...
function newPerson(index = 0): Person { return { id: `person-${Date.now()}-${index}`, name: index === 0 ? "Максим" : `Человек ${index + 1}`, daily: { ...defaultMacros }, macroPreset: "balanced", mealsPerDay: 4, includedSlots: ["breakfast", "lunch", "dinner"] }; }
...
export default function Home() {
  ...
  return <main className="app-shell"><div className="ambient ambient-one" /><div className="ambient ambient-two" /><div className="ambient ambient-three" /><header className="app-header"><div><p className="kicker">{currentTitle.kicker}</p><h1>{currentTitle.title}</h1></div><button className="avatar glass" onClick={() => navigate("profile")} aria-label="Открыть профиль">М</button></header>{tab === "week" && <WeekScreen key={activePlan?.id ?? "empty"} plan={activePlan} loading={loadingPlan} loadError={loadError} onBuild={() => navigate("builder")} onEditMenu={editDayMenu} onOpenRecipe={setRecipeContext} />}{tab === "recipes" && <RecipesScreen onOpenRecipe={(recipe) => setRecipeContext({ recipe })} />}{tab === "shopping" && <ShoppingScreen plan={activePlan} onBuild={() => navigate("builder")} onChange={async (plan) => { setActivePlan(plan); try { await persistPlan(plan); } catch { /* optimistic state */ } }} />}{tab === "profile" && <ProfileScreen people={activePlan?.people ?? [newPerson()]} onConfigure={editPeople} onOpenTutorial={() => { setOnboardingReturnTab("profile"); setOnboardingStep("welcome"); }} onOpenPrepGuide={() => { setOnboardingReturnTab("profile"); setOnboardingStep("prep-guide"); }} />}<BottomNav tab={tab} onNavigate={navigate} /></main>;
}

function OnboardingScreen({ step, hasPlan, onNext, onBack, onFinish }: { step: Exclude<OnboardingStep, "done">; hasPlan: boolean; onNext: () => void; onBack: () => void; onFinish: () => void }) {
...
function ProfileScreen({ people, onConfigure, onOpenTutorial }: { people: Person[]; onConfigure: () => void; onOpenTutorial: () => void }) {
  return <section className="screen profile-screen"><section className="profile-hero glass-card"><div className="large-avatar">М</div><div><p className="kicker">Ваше пространство</p><h2>{people.length} {people.length === 1 ? "человек" : "человека"}</h2><p>Цели используются для расчёта каждой порции.</p></div></section><div className="section-heading"><div><p className="kicker">Участники плана</p><h2>КБЖУ и режим</h2></div><button className="text-button" aria-label="Настроить людей и цели" onClick={onConfigure}>Настроить</button></div>{people.map((person, index) => { const coverage = Math.round(person.includedSlots.reduce((sum, slot) => sum + shareFor(person, slot), 0) * 100); return <section className="person-summary glass-card" key={person.id}><div className={`person-dot tone-${index}`}>{person.name.slice(0, 1)}</div><div className="person-main"><h3>{person.name}</h3><p>{person.mealsPerDay} приёма в день · в плане {coverage}%</p><div className="mini-macros">{(["kcal", "protein", "fat", "carbs"] as MacroKey[]).map((key) => <span key={key}><b>{person.daily[key]}</b> {macroLabels[key]}</span>)}</div></div></section>; })}<button className="tutorial-entry glass-card" onClick={onOpenTutorial}><span>?</span><div><b>Как работает Mise</b><small>Ещё раз открыть короткий инструктаж</small></div><i>›</i></button></section>;
}
...
function PlanBuilder({ initialPlan, initialStep = 0, initialBatchId, onClose, onSaved, persistPlan }: { initialPlan: ActivePlan | null; initialStep?: number; initialBatchId?: string; onClose: () => void; onSaved: (plan: ActivePlan, destination: Tab) => void; persistPlan: (plan: ActivePlan) => Promise<void> }) {
  ...
  function updateMacro(id: string, key: MacroKey, value: number) { setPeople((current) => current.map((person) => { if (person.id !== id) return person; const safeValue = Math.max(0, value); if (key === "kcal") { const preset = person.macroPreset ?? "balanced"; return { ...person, daily: recalculateDailyMacros(safeValue, person.daily, preset), macroPreset: preset }; } return { ...person, daily: { ...person.daily, [key]: safeValue }, macroPreset: "custom" }; })); invalidateMenu(); }
  function applyMacroPreset(id: string, preset: MacroPresetOption) { setPeople((current) => current.map((person) => person.id === id ? { ...person, daily: macrosForCalories(person.daily.kcal, preset), macroPreset: preset } : person)); invalidateMenu(); }
...
function PeopleStep({ people, mealSlots, onUpdate, onMacro, onPreset, onAdd, onRemove }: { people: Person[]; mealSlots: MealSlot[]; onUpdate: (id: string, patch: Partial<Person>) => void; onMacro: (id: string, key: MacroKey, value: number) => void; onPreset: (id: string, preset: MacroPresetOption) => void; onAdd: () => void; onRemove: (id: string) => void }) {
  ...
  return <><StepIntro icon="◎" kicker="Для кого готовим" title="Люди и цели" text="Введите калории и выберите распределение — БЖУ пересчитаются автоматически." />{people.map((person, index) => {
    const covered = person.includedSlots.reduce((sum, slot) => sum + shareFor(person, slot), 0);
    ...
    return <section className="person-editor glass-card" key={person.id}>
      ...
      <div className="meal-count"><span>Приёмов пищи в день</span><div>{[1, 2, 3, 4, 5].map((count) => <button key={count} className={person.mealsPerDay === count ? "selected" : ""} onClick={() => onUpdate(person.id, { mealsPerDay: count, includedSlots: person.includedSlots.slice(0, count) })}>{count}</button>)}</div></div>
      ...
      <div className="coverage-card"><div><span>В плане</span><b>{Math.round(covered * 100)}%</b></div><div><span>Вне плана</span><b>{Math.max(0, 100 - Math.round(covered * 100))}%</b></div></div>
      ...
      {person.includedSlots.length > person.mealsPerDay && <p className="inline-warning">Выбрано больше приёмов, чем указано для дня.</p>}
    </section>;
