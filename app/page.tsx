...
type IconName = "calendar" | "pot" | "basket" | "person" | "plus" | "chevron" | "check" | "clock";
...
function WeekScreen({ plan, loading, loadError, onBuild, onRepeat, onEditMenu, onOpenRecipe }: { plan: ActivePlan | null; loading: boolean; loadError: boolean; onBuild: () => void; onRepeat: () => void; onEditMenu: (batchId: string) => void; onOpenRecipe: (context: RecipeContext) => void }) {
  ...
  return <section className="screen week-screen">
    ...
    <div className="date-strip" ref={stripRef} aria-label="Дни плана">{dates.map((date) => { const dateBatch = batchFor(date); const isFirstOfBatch = dateBatch.start === date && dateBatch.index > 0; return <button key={date} data-selected={date === selectedDate} aria-pressed={date === selectedDate} className={`${date === selectedDate ? "selected" : ""} ${date === today ? "is-today" : ""} ${isFirstOfBatch ? "batch-start" : ""}`} onClick={() => setSelectedDate(date)} aria-label={`${formatDate(date, true)}${date === today ? ", сегодня" : ""}${dateBatch.start === date ? `, начало готовки ${dateBatch.index + 1}` : ""}`}><small>{new Intl.DateTimeFormat("ru-RU", { weekday: "short" }).format(parseDate(date)).replace(".", "")}</small><b>{parseDate(date).getDate()}</b>{date === today && <i className="today-dot" aria-hidden />}</button>; })}</div>
...
function ShoppingScreen({ plan, onBuild, onChange }: { plan: ActivePlan | null; onBuild: () => void; onChange: (plan: ActivePlan) => Promise<boolean> }) {
  ...
  return <section className="screen shopping-screen">
    ...
    {Object.entries(groups).map(([group, items]) => <section className="shopping-group glass-card" key={group}><div className="group-title"><h3>{group}</h3><span>{items.length}</span></div>{items.map((item) => <button className={`grocery-row ${item.checked ? "checked" : ""}`} key={item.key} aria-pressed={item.checked} onClick={() => toggle(item.key)}><span className="checkmark">{item.checked && <Icon name="check" />}</span><span className="grocery-name">{item.name}</span><b>{item.quantity.toLocaleString("ru-RU")} {item.unit}</b></button>)}</section>)}
  </section>;
}
...
function PlanBuilder({ initialPlan, initialStep = 0, initialBatchId, repeat = false, onClose, onSaved, persistPlan }: { initialPlan: ActivePlan | null; initialStep?: number; initialBatchId?: string; repeat?: boolean; onClose: () => void; onSaved: (plan: ActivePlan, destination: Tab) => void; persistPlan: (plan: ActivePlan) => Promise<void> }) {
  ...
  return <main className="app-shell builder-shell">
    ...
    <header className="builder-header">
      <button className="icon-button glass" onClick={step === initialStep ? closeBuilder : () => setStep((value) => value - 1)} aria-label={step === initialStep ? "Вернуться к неделе" : "Назад"}>{initialStep === 0 && step === 0 ? "×" : "‹"}</button>
...
function PeopleStep({ people, mealSlots, onUpdate, onMacro, onPreset, onAdd, onRemove }: { people: Person[]; mealSlots: MealSlot[]; onUpdate: (id: string, patch: Partial<Person>) => void; onMacro: (id: string, key: MacroKey, value: number) => void; onPreset: (id: string, preset: MacroPresetOption) => void; onAdd: () => void; onRemove: (id: string) => void }) {
  ...
  return <><StepIntro icon="◎" kicker="Для кого готовим" title="Люди и цели" text="Введите калории и выберите распределение — БЖУ пересчитаются автоматически." />{people.map((person, index) => {
    ...
    return <section className="person-editor glass-card" key={person.id}>
      ...
      <div className="goal-mode" role="group" aria-label={`Как задать цель для ${person.name || "человека"}`}>{([["manual", "Знаю свои КБЖУ"], ["estimate", "Помогите рассчитать"]] as const).map(([value, label]) => { const active = (modes[person.id] ?? (person.estimate ? "estimate" : "manual")) === value; return <button key={value} aria-pressed={active} className={active ? "selected" : ""} onClick={() => setModes((current) => ({ ...current, [person.id]: value }))}>{label}</button>; })}</div>
      ...
      <div className="macro-presets"><div className="macro-preset-heading"><p><b>Распределить калории</b><small>При изменении калорий БЖУ обновятся сами</small></p>{selectedPreset === "custom" && <em>Вручную</em>}</div><div className="macro-preset-grid">{presetOptions.map((preset) => { const meta = macroPresetMeta[preset]; const selected = selectedPreset === preset; return <button key={preset} className={selected ? "selected" : ""} aria-pressed={selected} onClick={() => onPreset(person.id, preset)}><b>{meta.label}</b><small>{meta.description}</small></button>; })}</div><p className="macro-preset-note">Доли считаются от калорий. Это ориентир, не медицинская рекомендация; любое БЖУ можно поправить вручную.</p></div>
      ...
      <div className="person-slots"><p>Что из плана ест {person.name || "человек"}</p><div>{mealSlots.map((slot) => { const active = person.includedSlots.includes(slot); return <button key={slot} aria-pressed={active} className={active ? "selected" : ""} onClick={() => onUpdate(person.id, { includedSlots: active ? person.includedSlots.filter((item) => item !== slot) : [...person.includedSlots, slot] })}>{active ? "✓ " : "+ "}{mealMeta[slot].short}</button>; })}</div></div>
...
function GoalEstimator({ person, onApply }: { person: Person; onApply: (estimate: GoalEstimate, kcal: number) => void }) {
  ...
  return <div className="goal-estimator">
    <div className="estimator-sex" role="group" aria-label="Пол">{(["male", "female"] as Sex[]).map((value) => <button key={value} aria-pressed={draft.sex === value} className={draft.sex === value ? "selected" : ""} onClick={() => setDraft({ ...draft, sex: value })}>{value === "male" ? "Мужчина" : "Женщина"}</button>)}</div>
...
function SuccessSheet({ plan, onOpen, onEdit }: { plan: ActivePlan; onOpen: (tab: Tab) => void; onEdit: () => void }) {
  ...
  return <div className="modal-backdrop"><section ref={dialogRef} tabIndex={-1} className={`success-sheet glass ${phase === "notifications" ? "notification-modal" : ""}`} role="dialog" aria-modal="true" aria-labelledby="success-title">{phase === "summary" ? <><div className="success-burst">✓</div><p className="kicker">Всё получилось</p><h2 id="success-title">План готов!</h2><p>Дней: {plan.periodDays} · готовок: {plan.batches.length} · рецептов: {new Set(Object.values(plan.selections)).size} · продуктов: {plan.shopping.length}</p><InstallInline /><button className="primary-button" onClick={() => setPhase("notifications")}>Настроить напоминания <span>→</span></button><button className="secondary-button" onClick={() => onOpen("week")}>Открыть план без них</button><button className="text-button" onClick={onEdit}>Изменить план</button></> : <NotificationSetupPanel plan={notificationPlanFor(plan)} clientId={clientId()} deviceId={deviceId()} onDone={() => onOpen("week")} onCancel={() => onOpen("week")} />}</section></div>;
}
