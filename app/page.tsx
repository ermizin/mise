...
import { countRu, pluralRu } from "./format";
...
type ActivityKey = "low" | "light" | "medium" | "high" | "athlete";
type GoalKey = "lose" | "keep" | "gain";
type GoalEstimate = { sex: Sex; age: number; height: number; weight: number; activity: ActivityKey; goal: GoalKey };
type Person = { id: string; name: string; daily: Macros; macroPreset?: MacroPreset; includedSlots: MealSlot[]; estimate?: GoalEstimate };
...
type BuilderEntry = { step: number; batchId?: string; returnTab?: Exclude<Tab, "builder">; repeat?: boolean };
type BuilderDraft = { planId: string | null; savedAt: number; step: number; choiceIndex: number; start: string; end: string; mealSlots: MealSlot[]; menuStyle: MenuStyle; people: Person[]; cookEveryDays: number; remainderDecision: "separate" | "extend" | "shorten" | null; selections: Record<string, string> };
type OnboardingStep = "welcome" | "guide" | "prep-offer" | "prep-guide" | "done";
...
const mealMeta: Record<MealSlot, { label: string; short: string; icon: IconName }> = {
  breakfast: { label: "Завтрак", short: "Завтрак", icon: "sun" }, lunch: { label: "Обед", short: "Обед", icon: "pot" }, dinner: { label: "Ужин", short: "Ужин", icon: "moon" }, snack1: { label: "Перекус 1", short: "Перекус 1", icon: "apple" }, snack2: { label: "Перекус 2", short: "Перекус 2", icon: "cup" },
};
const mealEmoji: Record<MealSlot, string> = { breakfast: "🥣", lunch: "🍲", dinner: "🍛", snack1: "🍎", snack2: "🥛" };
const styleMeta: Record<MenuStyle, { label: string; icon: IconName; description: string }> = {
  protein: { label: "Высокобелковое", icon: "target", description: "Больше белка для сытости и восстановления" }, budget: { label: "Бюджетное", icon: "wallet", description: "Простые продукты и разумная стоимость" }, paleo: { label: "Палео", icon: "leaf", description: "Мясо, рыба, овощи — без зерновых" }, keto: { label: "Кето", icon: "leaf", description: "Меньше углеводов, больше полезных жиров" },
};
...
const activityMeta: Record<ActivityKey, { label: string; factor: number }> = {
  low: { label: "Сидячий образ жизни", factor: 1.2 },
  light: { label: "1–2 тренировки в неделю", factor: 1.375 },
  medium: { label: "3–4 тренировки в неделю", factor: 1.55 },
  high: { label: "5–6 тренировок в неделю", factor: 1.725 },
  athlete: { label: "Ежедневные нагрузки или физический труд", factor: 1.9 },
};
const goalMeta: Record<GoalKey, { label: string; shift: number }> = {
  lose: { label: "Снижение веса — дефицит ≈ 15%", shift: -0.15 },
  keep: { label: "Поддержание веса", shift: 0 },
  gain: { label: "Набор массы — профицит ≈ 10%", shift: 0.1 },
};
...
for (const style of Object.keys(generatedTitles) as MenuStyle[]) for (const slot of Object.keys(mealMeta) as MealSlot[]) generatedTitles[style][slot].forEach((title, index) => recipes.push(r(`gen-${style}-${slot}-${index}`, slot, title, mealEmoji[slot], 18 + index * 3, scaleMacros(generatedMacros[style][slot], 0.94 + index * 0.03), slot.startsWith("snack") ? 240 : 410, style === "budget" ? 105 + index * 9 : 175 + index * 18, [style], ingredientsForTitle(title, generatedIngredients[style][slot], style), commonSteps, 4, generatedRecipeFreezable(title), { provenance: { kind: "generated", basedOn: generatedReferences[style] } })));
...
function estimateCalories(estimate: GoalEstimate) {
  const base = 10 * estimate.weight + 6.25 * estimate.height - 5 * estimate.age + (estimate.sex === "male" ? 5 : -161);
  const total = base * activityMeta[estimate.activity].factor * (1 + goalMeta[estimate.goal].shift);
...
function newPerson(index = 0): Person { return { id: `person-${Date.now()}-${index}`, name: index === 0 ? "Я" : `Человек ${index + 1}`, daily: { ...defaultMacros }, macroPreset: "balanced", includedSlots: ["breakfast", "lunch", "dinner"] }; }
...
export default function Home() {
  ...
  // eslint-disable-next-line react-hooks/set-state-in-effect -- bootstraps onboarding state and the stored plan on mount
  ...
  function repeatPlan() { setRecipeContext(null); setBuilderEntry({ step: 0, repeat: true }); setTab("builder"); }
  ...
  function completeCoreOnboarding() { localStorage.setItem(onboardingStorageKey, "complete"); setOnboardingStep("prep-offer"); }
  function finishOnboarding() { localStorage.setItem(onboardingStorageKey, "complete"); localStorage.setItem(prepGuideStorageKey, "complete"); setOnboardingStep("done"); const destination = onboardingReturnTab ?? (activePlan ? "week" : "builder"); setOnboardingReturnTab(null); navigate(destination); }
  if (onboardingStep !== "done") return <OnboardingScreen step={onboardingStep} hasPlan={Boolean(activePlan)} onNext={() => setOnboardingStep("guide")} onCoreComplete={completeCoreOnboarding} onShowPrepGuide={() => setOnboardingStep("prep-guide")} onBack={() => setOnboardingStep(onboardingStep === "prep-guide" ? "prep-offer" : "welcome")} onFinish={finishOnboarding} />;
  ...
  if (tab === "builder") return <PlanBuilder initialPlan={activePlan} initialStep={builderEntry.step} initialBatchId={builderEntry.batchId} repeat={builderEntry.repeat} onClose={() => navigate(builderEntry.returnTab ?? "week")} onSaved={(plan, destination) => { setActivePlan(plan); navigate(destination); }} persistPlan={persistPlan} />;
  ...
  return <main className="app-shell"><div className="ambient ambient-one" /><div className="ambient ambient-two" /><div className="ambient ambient-three" /><header className="app-header"><div><p className="kicker">{currentTitle.kicker}</p><h1>{currentTitle.title}</h1></div><button className="avatar glass" onClick={() => navigate("profile")} aria-label="Открыть профиль">М</button></header>{tab === "week" && <WeekScreen key={activePlan?.id ?? "empty"} plan={activePlan} loading={loadingPlan} loadError={loadError} onBuild={() => navigate("builder")} onRepeat={repeatPlan} onEditMenu={editDayMenu} onOpenRecipe={setRecipeContext} />}{tab === "recipes" && <RecipesScreen onOpenRecipe={(recipe) => setRecipeContext({ recipe })} />}{tab === "shopping" && <ShoppingScreen plan={activePlan} onBuild={() => navigate("builder")} onChange={async (next) => { const previous = activePlan; setActivePlan(next); try { await persistPlan(next); return true; } catch { if (previous) setActivePlan(previous); return false; } }} />}{tab === "profile" && <ProfileScreen people={activePlan?.people ?? [newPerson()]} hasPlan={Boolean(activePlan)} onConfigure={editPeople} onOpenTutorial={() => { setOnboardingReturnTab("profile"); setOnboardingStep("welcome"); }} onOpenPrepGuide={() => { setOnboardingReturnTab("profile"); setOnboardingStep("prep-guide"); }} onNotifications={() => setNotificationSetupOpen(true)} />}{activePlan && notificationSetupOpen && <Modal labelledBy="notifications-title" className="notification-modal" onClose={() => setNotificationSetupOpen(false)}><NotificationSetupPanel plan={notificationPlanFor(activePlan)} clientId={clientId()} deviceId={deviceId()} onDone={() => setNotificationSetupOpen(false)} onCancel={() => setNotificationSetupOpen(false)} /></Modal>}<BottomNav tab={tab} onNavigate={navigate} /></main>;
}

function OnboardingScreen({ step, hasPlan, onNext, onCoreComplete, onShowPrepGuide, onBack, onFinish: onDone }: { step: Exclude<OnboardingStep, "done">; hasPlan: boolean; onNext: () => void; onCoreComplete: () => void; onShowPrepGuide: () => void; onBack: () => void; onFinish: () => void }) {
...
function InstallInline() {
  ...
  async function install() { if (!prompt) { setShowSteps(true); return; } await prompt.prompt(); await prompt.userChoice; setPrompt(null); }
  if (installed) return null;
  return <section className="install-inline glass-card">
    <div className="install-inline-copy"><b>Добавить Mise на экран Домой</b><small>Открывается как приложение и может присылать напоминания о покупках, готовке и разморозке.</small>{showSteps && <p>{isIos ? "В Safari: «Поделиться» → «На экран Домой» → «Добавить»." : "В меню браузера выберите «Установить приложение» или «На главный экран»."}</p>}</div>
    <button className="secondary-button" onClick={install}>{prompt ? "Добавить" : "Как добавить"}</button>
  </section>;
}

function PrepGuideOffer({ hasPlan, onShow, onSkip }: { hasPlan: boolean; onShow: () => void; onSkip: () => void }) {
  return <main className="app-shell onboarding-shell prep-offer-shell"><div className="ambient ambient-one" /><div className="ambient ambient-two" /><div className="ambient ambient-three" /><header className="onboarding-header"><div className="mise-mark" aria-label="Mise">M</div><span>mise</span><button className="text-button" onClick={onSkip}>Не сейчас</button></header><section className="prep-offer"><div className="prep-offer-visual" aria-hidden><div className="prep-offer-main">🥣</div><span className="prep-float prep-float-containers"><Icon name="container" /> Контейнеры</span><span className="prep-float prep-float-cooking"><Icon name="pot" /> Готовка</span><span className="prep-float prep-float-labels"><Icon name="check" /> Подписи</span></div><p className="kicker">Перед первым милпрепом</p><h1>Нужна инструкция<br />по милпрепу?</h1><p className="onboarding-lead">За пару минут покажем, как подготовить контейнеры, организовать готовку, разложить порции и хранить их.</p><div className="prep-topic-row"><span>Контейнеры</span><span>Готовка</span><span>Раскладка</span><span>Хранение</span></div><div className="onboarding-actions"><button className="primary-button" onClick={onShow}>Да, показать <span>→</span></button><button className="text-button" onClick={onSkip}>{hasPlan ? "Вернуться к плану" : "Нет, составить план"}</button></div></section></main>;
}

function MealPrepGuide({ hasPlan, onBack, onFinish }: { hasPlan: boolean; onBack: () => void; onFinish: () => void }) {
  const steps: { icon: IconName; title: string; text: string }[] = [
    { icon: "container", title: "Подготовьте контейнеры", text: "По одному контейнеру на каждую порцию, подходящие крышки и наклейки или маркер для подписей." },
    { icon: "pot", title: "Готовьте партиями", text: "Сверьтесь со списком покупок, начните с самых долгих блюд и следуйте шагам в карточках рецептов." },
    { icon: "check", title: "Охладите и разложите", text: "Не держите готовую еду надолго в тепле. Разделите её по рассчитанным Mise порциям." },
    { icon: "snowflake", title: "Подпишите и уберите", text: "Укажите имя, дату и приём пищи. Ближние порции храните в холодильнике, остальные заморозьте по подсказке плана." },
  ];
  return <main className="app-shell onboarding-shell"><div className="ambient ambient-one" /><div className="ambient ambient-two" /><div className="ambient ambient-three" /><header className="onboarding-header"><div className="mise-mark" aria-label="Mise">M</div><span>mise</span><button className="text-button" onClick={onFinish}>Закрыть</button></header><section className="onboarding-guide prep-guide"><p className="kicker">Практическая инструкция</p><h1>От продуктов до готовых контейнеров</h1><p className="onboarding-lead">Простой порядок, чтобы не считать и не вспоминать всё во время готовки.</p><div className="prep-checklist">{steps.map((item, index) => <article className="prep-guide-card glass-card" key={item.title}><span className="prep-step-number">{index + 1}</span><div className="prep-step-icon"><Icon name={item.icon} /></div><div><h2>{item.title}</h2><p>{item.text}</p></div></article>)}</div><aside className="estimate-note prep-storage-note"><span><Icon name="info" /></span><p><b>Для каждого блюда — свои условия</b><small>Проверьте срок, способ хранения и разморозку в карточке конкретного рецепта.</small></p></aside><div className="onboarding-actions"><button className="primary-button" onClick={onFinish}>{hasPlan ? "Вернуться к плану" : "Составить первый план"} <span>→</span></button><button className="text-button" onClick={onBack}>Назад</button></div></section></main>;
}

function Modal({ labelledBy, className = "", onClose, children }: { labelledBy: string; className?: string; onClose: () => void; children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const siblings = backdropRef.current?.parentElement
      ? Array.from(backdropRef.current.parentElement.children).filter((node): node is HTMLElement => node instanceof HTMLElement && node !== backdropRef.current)
      : [];
  ...
  return <div ref={backdropRef} className="modal-backdrop"><button className="modal-dismiss" tabIndex={-1} aria-label="Закрыть диалог" onClick={onClose} /><section ref={dialogRef} tabIndex={-1} className={`success-sheet glass ${className}`} role="dialog" aria-modal="true" aria-labelledby={labelledBy}>{children}</section></div>;
}

function recipeTone(recipe: Recipe) {
  return [...recipe.id].reduce((sum, character) => sum + character.charCodeAt(0), 0) % 5;
}

function formatMacros(macros: Macros) {
  return `${macros.kcal} ккал · Б ${macros.protein} г · Ж ${macros.fat} г · У ${macros.carbs} г`;
}

function RecipeArtwork({ recipe, className, badge, sourcePhoto = false }: { recipe: Recipe; className: string; badge?: string; sourcePhoto?: boolean }) {
  const [photoFailed, setPhotoFailed] = useState(false);
  const image = sourcePhoto && recipe.provenance.kind === "parsed" && recipe.provenance.imageUrl && !photoFailed ? recipe.provenance.imageUrl : null;
  return <div className={`${className} art-${recipeTone(recipe)} ${image ? "has-photo" : ""}`}>{image ? <img src={image} alt={recipe.provenance.kind === "parsed" ? recipe.provenance.imageAlt ?? recipe.title : recipe.title} loading="lazy" referrerPolicy="no-referrer" onError={() => setPhotoFailed(true)} /> : <span>{recipe.emoji}</span>}{badge && <em>{badge}</em>}</div>;
}

function handleRadioKeyDown(event: ReactKeyboardEvent<HTMLElement>) {
...
function BottomNav({ tab, onNavigate }: { tab: Tab; onNavigate: (tab: Tab) => void }) {
  const items: { id: Exclude<Tab, "builder">; label: string; full: string; icon: IconName }[] = [{ id: "week", label: "Неделя", full: "План на неделю", icon: "calendar" }, { id: "recipes", label: "Рецепты", full: "Рецепты", icon: "pot" }, { id: "shopping", label: "Покупки", full: "Покупки", icon: "basket" }, { id: "profile", label: "Профиль", full: "Профиль", icon: "person" }];
  return <><button className="compose-fab" onClick={() => onNavigate("builder")} aria-label="Составить план"><Icon name="plus" /><small>Составить</small></button><nav className="bottom-nav glass" aria-label="Основная навигация">{items.map((item) => <button key={item.id} className={tab === item.id ? "is-active" : ""} aria-label={item.full} aria-current={tab === item.id ? "page" : undefined} onClick={() => onNavigate(item.id)}><Icon name={item.icon} /><small>{item.label}</small></button>)}</nav></>;
}

function EmptyState({ onBuild, title, text }: { onBuild: () => void; title: string; text: string }) { return <section className="empty-state glass-card"><div className="empty-orbit"><span><Icon name="sparkles" /></span></div><p className="kicker">Персональный милпреп</p><h2>{title}</h2><p>{text}</p><button className="primary-button" onClick={onBuild}>Составить план <span>→</span></button></section>; }
...
function WeekScreen({ plan, loading, loadError, onBuild, onRepeat, onEditMenu, onOpenRecipe }: { plan: ActivePlan | null; loading: boolean; loadError: boolean; onBuild: () => void; onRepeat: () => void; onEditMenu: (batchId: string) => void; onOpenRecipe: (context: RecipeContext) => void }) {
  const today = isoDate(new Date());
  const [selectedDate, setSelectedDate] = useState(plan ? clampDate(today, plan.start, plan.end) : today);
  const [personId, setPersonId] = useState(plan?.people[0]?.id ?? "");
  const stripRef = useRef<HTMLDivElement | null>(null);
  useEffect(() => { stripRef.current?.querySelector<HTMLElement>("[data-selected='true']")?.scrollIntoView({ inline: "center", block: "nearest", behavior: "smooth" }); }, [selectedDate]);
  ...
  const batchFor = (date: string) => activePlan.batches.find((item) => date >= item.start && date <= item.end) ?? activePlan.batches[0];
  ...
  const daysToNextCook = nextCook ? daysInclusive(today, nextCook.start) - 1 : 0;
  const tomorrow = addDays(today, 1);
  const thawTitles = planEnded || tomorrow > plan.end || tomorrow < plan.start ? [] : (() => { const nextBatch = batchFor(tomorrow); const index = daysInclusive(nextBatch.start, tomorrow) - 1; return plan.mealSlots.flatMap((slot) => { const recipe = recipesById[activePlan.selections[selectionKey(nextBatch, slot)]]; return recipe && recipe.freezable && index + 1 > recipe.storageDays ? [recipe.title] : []; }); })();
  const todayMealCount = planEnded || notStarted ? 0 : plan.mealSlots.filter((slot) => recipesById[activePlan.selections[selectionKey(batchFor(today), slot)]]).length;
  return <section className="screen week-screen">
    {planEnded ? <section className="today-card glass-card ended" role="status">
      <p className="kicker">План завершён</p>
      <h2>{formatDate(plan.start)} — {formatDate(plan.end)}</h2>
      <p>Период закончился. Соберите следующий, пока не пришлось снова всё пересчитывать.</p>
      <div className="today-actions"><button className="primary-button" onClick={onRepeat}>Повторить план <span>→</span></button><button className="secondary-button" onClick={onBuild}>Составить новый</button></div>
    </section> : <section className="today-card glass-card" role="status">
      <p className="kicker">{notStarted ? `План начнётся ${formatDate(plan.start)}` : cookToday ? "Сегодня — день готовки" : "Сегодня"}</p>
      <h2>{notStarted ? "Пока ничего доставать не нужно" : cookToday ? `Готовим партию ${todayBatch!.index + 1} на ${todayBatch!.days} дн.` : `${todayMealCount} ${todayMealCount === 1 ? "контейнер" : todayMealCount < 5 ? "контейнера" : "контейнеров"} из холодильника`}</h2>
      <p>{nextCook ? `Следующая готовка ${formatDate(nextCook.start)} — через ${daysToNextCook} ${daysToNextCook === 1 ? "день" : daysToNextCook < 5 ? "дня" : "дней"}.` : "Это последняя партия периода."}</p>
      {thawTitles.length > 0 && <p className="today-thaw">Вечером переложите в холодильник: {thawTitles.join(", ")}.</p>}
      {selectedDate !== clampDate(today, plan.start, plan.end) && <button className="text-button" onClick={() => setSelectedDate(clampDate(today, activePlan.start, activePlan.end))}>Показать сегодняшний день</button>}
    </section>}
    <div className="date-strip" ref={stripRef} role="radiogroup" tabIndex={-1} onKeyDown={handleRadioKeyDown} aria-label="Дни плана">{dates.map((date) => { const dateBatch = batchFor(date); const isFirstOfBatch = dateBatch.start === date && dateBatch.index > 0; return <button key={date} role="radio" data-selected={date === selectedDate} aria-checked={date === selectedDate} className={`${date === selectedDate ? "selected" : ""} ${date === today ? "is-today" : ""} ${isFirstOfBatch ? "batch-start" : ""}`} onClick={() => setSelectedDate(date)} aria-label={`${formatDate(date, true)}${date === today ? ", сегодня" : ""}${dateBatch.start === date ? `, начало готовки ${dateBatch.index + 1}` : ""}`}><small>{new Intl.DateTimeFormat("ru-RU", { weekday: "short" }).format(parseDate(date)).replace(".", "")}</small><b>{parseDate(date).getDate()}</b>{date === today && <i className="today-dot" aria-hidden />}</button>; })}</div>
    ...
    <button className="prep-callout glass-card" onClick={() => setSelectedDate(batch.start)} aria-label={`Открыть день готовки ${batch.index + 1}`}><Icon name="pot" className="prep-icon" /><div><p className="kicker">Готовка {batch.index + 1}</p><h3>{formatDate(batch.start)} — {formatDate(batch.end)}</h3><p>{batch.days} {batch.days === 1 ? "день" : batch.days < 5 ? "дня" : "дней"} · {dayMeals.length} блюд</p></div><Icon name="chevron" className="soft-chevron" /></button>
    <div className="section-heading"><div><p className="kicker">{formatDate(selectedDate, true)} · день {dayIndex + 1} из {batch.days}</p><h2>Меню дня</h2></div><button className="text-button" aria-label={`Изменить меню на ${formatDate(selectedDate, true)}`} onClick={() => onEditMenu(batch.id)}>Изменить</button></div>
    <div className="day-meals">{dayMeals.map(({ slot, recipe }) => { const portion = person?.includedSlots.includes(slot) ? portionFor(person, slot, recipe, plan.tuning?.[tuningKey(batch, slot, person)]) : null; const left = batch.days - dayIndex; const prepStatus = dayIndex === 0 ? (selectedDate === today ? "Готовить сегодня" : "День готовки") : `Разогреть · осталось ${countRu(left, "контейнер", "контейнера", "контейнеров")}`; return <button className="week-meal glass-card" key={`${slot}-${recipe.id}`} onClick={() => onOpenRecipe({ recipe, batch, slot, plan })}><RecipeArtwork recipe={recipe} className="food-art" /><div className="week-meal-copy"><p className="kicker">{prepStatus}</p><h3>{recipe.title}</h3>{portion ? <p>{formatMacros(portion.actual)} · около {portion.grams} г</p> : <p>Не входит в меню {person.name}</p>}</div><Icon name="chevron" className="soft-chevron" /></button>; })}</div>
  </section>;
}
...
function RecipesScreen({ onOpenRecipe }: { onOpenRecipe: (recipe: Recipe) => void }) {
  ...
  return <section className="screen recipes-screen"><div className="origin-segment" role="radiogroup" tabIndex={-1} onKeyDown={handleRadioKeyDown} aria-label="Происхождение рецепта"><button role="radio" aria-checked={origin === "parsed"} className={origin === "parsed" ? "selected" : ""} onClick={() => setOrigin("parsed")}>Спаршенные</button><button role="radio" aria-checked={origin === "generated"} className={origin === "generated" ? "selected" : ""} onClick={() => setOrigin("generated")}>Сгенерированные</button></div><div className="horizontal-pills" role="radiogroup" tabIndex={-1} onKeyDown={handleRadioKeyDown} aria-label="Тип меню">{(Object.keys(styleMeta) as MenuStyle[]).map((value) => <button key={value} role="radio" aria-checked={style === value} className={style === value ? "selected" : ""} onClick={() => setStyle(value)}><Icon name={styleMeta[value].icon} /> {styleMeta[value].label}</button>)}</div><div className="meal-segment" role="radiogroup" tabIndex={-1} onKeyDown={handleRadioKeyDown} aria-label="Приём пищи">{(Object.keys(mealMeta) as MealSlot[]).map((value) => <button key={value} role="radio" aria-checked={slot === value} className={slot === value ? "selected" : ""} onClick={() => setSlot(value)}>{mealMeta[value].label}</button>)}</div><div className="catalog-filters"><label>Сложность<select value={effort} onChange={(event) => setEffort(event.target.value as typeof effort)}><option value="all">Любая</option><option value="low">Низкая</option><option value="high">Высокая</option></select></label><label>Время<select value={time} onChange={(event) => setTime(event.target.value as typeof time)}><option value="all">Любое</option><option value="quick">До 20 мин</option><option value="medium">21–40 мин</option><option value="long">41+ мин</option></select></label></div><div className="section-heading"><div><p className="kicker">{countRu(visible.length, "вариант", "варианта", "вариантов")}</p><h2>{mealMeta[slot].label}</h2></div></div>{visible.length ? <div className="catalog-grid">{visible.map((recipe) => <button className="catalog-card glass-card" key={recipe.id} onClick={() => onOpenRecipe(recipe)}><RecipeArtwork recipe={recipe} className="catalog-art" badge={`${recipe.time} мин`} /><h3>{recipe.title}</h3><p>{formatMacros(recipe.macros)}</p><div className="recipe-badges"><span>{recipe.effort.level === "low" ? "Мало действий" : "Много действий"}</span><span>{recipe.storage.ambient ? `Сухая банка · ${countRu(recipe.storageDays, "день", "дня", "дней")}` : recipe.freezable ? "Морозилка" : `Холодильник · ${countRu(recipe.storageDays, "день", "дня", "дней")}`}</span>{recipe.localization.fit !== "familiar" && <span>{recipe.localization.fit === "adapted" ? "Адаптирован" : "Нишевый вкус"}</span>}</div><span className="round-arrow">↗</span></button>)}</div> : <section className="catalog-empty glass-card"><span><Icon name="search" /></span><h3>Пока нет совпадений</h3><p>Смените время, сложность или категорию.</p></section>}</section>;
}
...
function ShoppingScreen({ plan, onBuild, onChange }: { plan: ActivePlan | null; onBuild: () => void; onChange: (plan: ActivePlan) => Promise<boolean> }) {
  ...
  async function apply(shopping: ShoppingItem[], undoTo: ShoppingItem[] | null) {
    const ok = await onChange({ ...currentPlan, shopping });
    setFailed(!ok);
    setUndoItems(ok ? undoTo : null);
  }
  function toggle(key: string) { void apply(currentPlan.shopping.map((item) => item.key === key ? { ...item, checked: !item.checked } : item), null); }
  function clearChecks() { void apply(currentPlan.shopping.map((item) => ({ ...item, checked: false })), currentPlan.shopping); }
  function undo() { const restore = undoItems; if (restore) void apply(restore, null); }
  return <section className="screen shopping-screen">
    <section className="shopping-summary glass-card"><div><p className="kicker">Куплено {checked} из {plan.shopping.length}</p><h2>{Math.round((checked / Math.max(plan.shopping.length, 1)) * 100)}%</h2>{checked > 0 && <button className="text-button" onClick={clearChecks}>Снять отметки</button>}</div><div className="progress-ring" style={{ "--progress": `${Math.round((checked / Math.max(plan.shopping.length, 1)) * 100) * 3.6}deg` } as React.CSSProperties}><Icon name="check" /></div></section>
    {failed && <div className="save-error inline-error" role="alert">Отметка не сохранилась — проверьте связь и нажмите ещё раз.</div>}
    {undoItems && <div className="undo-bar" role="status"><span>Отметки сняты</span><button className="text-button" onClick={undo}>Вернуть</button></div>}
    {Object.entries(groups).map(([group, items]) => <section className="shopping-group glass-card" key={group}><div className="group-title"><h3>{group}</h3><span>{items.length}</span></div>{items.map((item) => <button className={`grocery-row ${item.checked ? "checked" : ""}`} key={item.key} role="checkbox" aria-checked={item.checked} onClick={() => toggle(item.key)}><span className="checkmark">{item.checked && <Icon name="check" />}</span><span className="grocery-name">{item.name}</span><b>{item.quantity.toLocaleString("ru-RU")} {item.unit}</b></button>)}</section>)}
  </section>;
}
...
function ProfileScreen({ people, hasPlan, onConfigure, onOpenTutorial, onOpenPrepGuide, onNotifications }: { people: Person[]; hasPlan: boolean; onConfigure: () => void; onOpenTutorial: () => void; onOpenPrepGuide: () => void; onNotifications: () => void }) {
  return <section className="screen profile-screen"><section className="profile-hero glass-card"><div className="large-avatar">М</div><div><p className="kicker">Ваше пространство</p><h2>{countRu(people.length, "человек", "человека", "человек")}</h2><p>Цели используются для расчёта каждой порции.</p></div></section><div className="section-heading"><div><p className="kicker">Участники плана</p><h2>КБЖУ и блюда</h2></div><button className="text-button" aria-label="Настроить людей и цели" onClick={onConfigure}>Настроить</button></div>{people.map((person, index) => { const planned = plannedTargetsFor(person); const difference = macroDifference(person.daily, planned); return <section className="person-summary glass-card" key={person.id}><div className={`person-dot tone-${index}`}>{person.name.slice(0, 1)}</div><div className="person-main"><h3>{person.name}</h3><p>{countRu(person.includedSlots.length, "позиция", "позиции", "позиций")} из Mise · {difference.kcal > 50 ? `ещё ≈ ${difference.kcal} ккал` : difference.kcal < -50 ? `выше цели на ≈ ${Math.abs(difference.kcal)} ккал` : "цель примерно закрыта"}</p><div className="mini-macros">{(["kcal", "protein", "fat", "carbs"] as MacroKey[]).map((key) => <span key={key}><b>{person.daily[key]}</b> {macroLabels[key]}</span>)}</div></div></section>; })}<InstallInline />{hasPlan && <button className="tutorial-entry notification-entry glass-card" onClick={onNotifications}><span><Icon name="bell" /></span><div><b>Напоминания</b><small>Время готовки, покупок и разморозки</small></div><Icon name="chevron" className="entry-chevron" /></button>}<button className="tutorial-entry glass-card" onClick={onOpenTutorial}><span><Icon name="info" /></span><div><b>Как работает Mise</b><small>Ещё раз открыть короткий онбординг</small></div><Icon name="chevron" className="entry-chevron" /></button><button className="tutorial-entry prep-tutorial-entry glass-card" onClick={onOpenPrepGuide}><span><Icon name="pot" /></span><div><b>Инструкция по милпрепу</b><small>Контейнеры, готовка, раскладка и хранение</small></div><Icon name="chevron" className="entry-chevron" /></button></section>;
}

function PlanBuilder({ initialPlan, initialStep = 0, initialBatchId, repeat = false, onClose, onSaved, persistPlan }: { initialPlan: ActivePlan | null; initialStep?: number; initialBatchId?: string; repeat?: boolean; onClose: () => void; onSaved: (plan: ActivePlan, destination: Tab) => void; persistPlan: (plan: ActivePlan) => Promise<void> }) {
  ...
  const today = isoDate(new Date()); const [step, setStep] = useState(initialStep); const [start, setStart] = useState(repeat ? today : initialPlan?.start ?? today); const [end, setEnd] = useState(repeat ? addDays(today, Math.max(0, (initialPlan?.periodDays ?? 7) - 1)) : initialPlan?.end ?? addDays(today, 6)); const [mealSlots, setMealSlots] = useState<MealSlot[]>(initialPlan?.mealSlots ?? ["breakfast", "lunch", "dinner"]); const [menuStyle, setMenuStyle] = useState<MenuStyle>(initialPlan?.menuStyle ?? "protein"); const [people, setPeople] = useState<Person[]>(initialPlan?.people ?? [{ ...newPerson(0), includedSlots: ["breakfast", "lunch", "dinner"] }]); const [cookEveryDays, setCookEveryDays] = useState(initialPlan?.cookEveryDays ?? 3); const [remainderDecision, setRemainderDecision] = useState<"separate" | "extend" | "shorten" | null>(initialPlan && initialPlan.periodDays % initialPlan.cookEveryDays ? "separate" : null); const [selections, setSelections] = useState<Record<string, string>>(initialPlan?.selections ?? {}); const [choiceIndex, setChoiceIndex] = useState(initialChoiceIndex); const [saveState, setSaveState] = useState<"idle" | "saving" | "error">("idle"); const [successPlan, setSuccessPlan] = useState<ActivePlan | null>(null);
  const [draftRestored, setDraftRestored] = useState(false);
  const stepRef = useRef(step);
  const closeRef = useRef(onClose);
  useEffect(() => { stepRef.current = step; closeRef.current = onClose; });
  function clearDraft() { try { localStorage.removeItem(builderDraftKey); } catch { /* storage may be unavailable */ } }
  function closeBuilder() { clearDraft(); history.back(); window.setTimeout(() => closeRef.current(), 250); }
  function discardDraft() { clearDraft(); setDraftRestored(false); setStep(initialStep); setStart(repeat ? today : initialPlan?.start ?? today); setEnd(repeat ? addDays(today, Math.max(0, (initialPlan?.periodDays ?? 7) - 1)) : initialPlan?.end ?? addDays(today, 6)); setMealSlots(initialPlan?.mealSlots ?? ["breakfast", "lunch", "dinner"]); setMenuStyle(initialPlan?.menuStyle ?? "protein"); setPeople(initialPlan?.people ?? [{ ...newPerson(0), includedSlots: ["breakfast", "lunch", "dinner"] }]); setCookEveryDays(initialPlan?.cookEveryDays ?? 3); setRemainderDecision(null); setSelections(initialPlan?.selections ?? {}); setChoiceIndex(initialChoiceIndex); }
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (sameTarget && fresh) {
          ...
          setStart(draft.start); setEnd(draft.end); setMealSlots(draft.mealSlots); setMenuStyle(draft.menuStyle);
          setPeople(draft.people); setCookEveryDays(draft.cookEveryDays); setRemainderDecision(draft.remainderDecision);
          setSelections(draft.selections); setStep(draft.step); setChoiceIndex(draft.choiceIndex);
          setDraftRestored(true);
        } else if (!fresh) localStorage.removeItem(builderDraftKey);
      }
    } catch { /* a broken draft must never block the wizard */ }
  ...
  useEffect(() => {
    ...
    const onPop = () => { if (stepRef.current > initialStep) { setStep((value) => value - 1); history.pushState({ mise: "builder" }, ""); } else closeRef.current(); };
  ...
  const allSelected = positions.every(({ batch, slot }) => Boolean(validSelections[selectionKey(batch, slot)])); const unassignedSlots = mealSlots.filter((slot) => !people.some((person) => person.includedSlots.includes(slot)));
  const staleCount = positions.filter(({ batch, slot }) => selections[selectionKey(batch, slot)] && !validSelections[selectionKey(batch, slot)]).length;
  useEffect(() => {
    if (successPlan) return;
    const draft: BuilderDraft = { planId: initialPlan?.id ?? null, savedAt: Date.now(), step, choiceIndex, start, end, mealSlots, menuStyle, people, cookEveryDays, remainderDecision, selections };
    try { localStorage.setItem(builderDraftKey, JSON.stringify(draft)); } catch { /* storage may be unavailable */ }
  }, [step, choiceIndex, start, end, mealSlots, menuStyle, people, cookEveryDays, remainderDecision, selections, successPlan, initialPlan]);
  ...
  async function save() { const plan: ActivePlan = { ...draftPlan, id: initialPlan?.id ?? crypto.randomUUID(), createdAt: initialPlan?.createdAt ?? new Date().toISOString() }; setSaveState("saving"); try { await persistPlan(plan); setSaveState("idle"); clearDraft(); setSuccessPlan(plan); } catch { setSaveState("error"); } }
  return <main className="app-shell builder-shell">
    <div className="ambient ambient-one" /><div className="ambient ambient-two" />
    <header className="builder-header">
      <button className="icon-button glass" onClick={step === initialStep ? closeBuilder : () => setStep((value) => value - 1)} aria-label={step === initialStep ? "Вернуться к неделе" : "Назад"}><Icon name={initialStep === 0 && step === 0 ? "close" : "chevron"} className={initialStep === 0 && step === 0 ? undefined : "back-chevron"} /></button>
      <div><p className="kicker">Шаг {step + 1} из {steps.length}</p><h1>{steps[step]}</h1></div>
    </header>
    <section className="builder-content">
      {draftRestored && <div className="inline-note draft-note" role="status"><p>Продолжаем незаконченный черновик плана.</p><button className="text-button" onClick={discardDraft}>Начать заново</button></div>}
      {staleCount > 0 && <p className="inline-note" role="status">{countRu(staleCount, "блюдо не подходит", "блюда не подходят", "блюд не подходят")} под новые настройки — выберите {staleCount === 1 ? "его" : "их"} заново на шаге «Выбор меню».</p>}
      ...
      {step === 2 && <StyleStep selected={menuStyle} onSelect={(value) => { setMenuStyle(value); }} />}
    ...
    <footer className="builder-actions glass">
      <button className="secondary-button" onClick={step === initialStep ? closeBuilder : () => setStep((value) => value - 1)}>{step === initialStep ? "Отмена" : "Назад"}</button>
...
function StepIntro({ icon, kicker, title, text }: { icon: IconName; kicker: string; title: string; text: string }) { return <div className="step-intro"><span><Icon name={icon} /></span><p className="kicker">{kicker}</p><h2>{title}</h2><p>{text}</p></div>; }
function PeriodStep({ start, end, rawDays, valid, onStart, onEnd, onQuick }: { start: string; end: string; rawDays: number; valid: boolean; onStart: (value: string) => void; onEnd: (value: string) => void; onQuick: (days: number) => void }) { return <><StepIntro icon="calendar" kicker="Когда едим" title="Выберите период" text="До 14 дней — так проще сохранить свежесть и разнообразие." /><section className="glass-card form-card"><div className="date-fields"><label>Начало<input type="date" value={start} onChange={(event) => onStart(event.target.value)} /></label><span>→</span><label>Конец<input type="date" value={end} onChange={(event) => onEnd(event.target.value)} /></label></div><div className="quick-periods" role="radiogroup" tabIndex={-1} onKeyDown={handleRadioKeyDown} aria-label="Быстрый выбор периода">{[3, 5, 7, 14].map((days) => <button key={days} role="radio" aria-checked={rawDays === days} className={rawDays === days ? "selected" : ""} onClick={() => onQuick(days)}>{countRu(days, "день", "дня", "дней")}</button>)}</div>{valid ? <div className="result-line"><span><Icon name="check" /></span><p><b>{formatDate(start)} — {formatDate(end)}</b><small>{countRu(rawDays, "день", "дня", "дней")} в плане</small></p></div> : <div className="warning-line">Период должен быть от 1 до 14 дней.</div>}</section></>; }
function MealStep({ selected, periodDays, onToggle }: { selected: MealSlot[]; periodDays: number; onToggle: (slot: MealSlot) => void }) { return <><StepIntro icon="pot" kicker="Что планируем" title="Выберите блюда из Mise" text="Отметьте только то, что хотите приготовить заранее. Остаток дневной цели мы покажем отдельно." /><div className="choice-grid meals-grid">{(Object.keys(mealMeta) as MealSlot[]).map((slot) => { const active = selected.includes(slot); return <button key={slot} role="checkbox" aria-checked={active} className={`choice-card glass-card ${active ? "selected" : ""}`} onClick={() => onToggle(slot)}><span className="choice-icon"><Icon name={mealMeta[slot].icon} /></span><b>{mealMeta[slot].label}</b><small>{active ? "Включён ✓" : "Добавить"}</small></button>; })}</div><section className="calculation-note glass-card"><span><Icon name="info" /></span><p><b>{countRu(selected.length, "позиция", "позиции", "позиций")} меню на день</b><small>{countRu(selected.length * periodDays, "порция", "порции", "порций")} на человека за период, если он ест все выбранные блюда</small></p></section></>; }
function StyleStep({ selected, onSelect }: { selected: MenuStyle; onSelect: (style: MenuStyle) => void }) { return <><StepIntro icon="sparkles" kicker="Какое меню" title="Выберите направление" text="Мы изменим порядок рекомендаций и покажем самые подходящие варианты первыми." /><div className="style-list" role="radiogroup" tabIndex={-1} onKeyDown={handleRadioKeyDown} aria-label="Направление меню">{(Object.keys(styleMeta) as MenuStyle[]).map((style) => <button key={style} role="radio" className={`style-card glass-card ${selected === style ? "selected" : ""}`} aria-checked={selected === style} onClick={() => onSelect(style)}><span><Icon name={styleMeta[style].icon} /></span><div><h3>{styleMeta[style].label}</h3><p>{styleMeta[style].description}</p></div><i>{selected === style ? "✓" : ""}</i></button>)}</div></>; }
...
function PeopleStep({ people, mealSlots, onUpdate, onMacro, onPreset, onAdd, onRemove }: { people: Person[]; mealSlots: MealSlot[]; onUpdate: (id: string, patch: Partial<Person>) => void; onMacro: (id: string, key: MacroKey, value: number) => void; onPreset: (id: string, preset: MacroPresetOption) => void; onAdd: () => void; onRemove: (id: string) => void }) {
  ...
  const [modes, setModes] = useState<Record<string, "manual" | "estimate">>({});
  return <><StepIntro icon="person" kicker="Для кого готовим" title="Люди и цели" text="Введите калории и выберите распределение — БЖУ пересчитаются автоматически." />{people.map((person, index) => {
    ...
    return <section className="person-editor glass-card" key={person.id}>
      ...
      <div className="goal-mode" role="radiogroup" tabIndex={-1} onKeyDown={handleRadioKeyDown} aria-label={`Как задать цель для ${person.name || "человека"}`}>{([["manual", "Знаю свои КБЖУ"], ["estimate", "Помогите рассчитать"]] as const).map(([value, label]) => { const active = (modes[person.id] ?? (person.estimate ? "estimate" : "manual")) === value; return <button key={value} role="radio" aria-checked={active} className={active ? "selected" : ""} onClick={() => setModes((current) => ({ ...current, [person.id]: value }))}>{label}</button>; })}</div>
      {(modes[person.id] ?? (person.estimate ? "estimate" : "manual")) === "estimate" && <GoalEstimator person={person} onApply={(estimate, kcal) => { onUpdate(person.id, { estimate }); onMacro(person.id, "kcal", kcal); }} />}
      ...
      <div className="macro-presets"><div className="macro-preset-heading"><p><b>Распределить калории</b><small>При изменении калорий БЖУ обновятся сами</small></p>{selectedPreset === "custom" && <em>Вручную</em>}</div><div className="macro-preset-grid" role="radiogroup" tabIndex={-1} onKeyDown={handleRadioKeyDown} aria-label={`Распределение калорий для ${person.name}`}>{presetOptions.map((preset) => { const meta = macroPresetMeta[preset]; const selected = selectedPreset === preset; return <button key={preset} role="radio" className={selected ? "selected" : ""} aria-checked={selected} onClick={() => onPreset(person.id, preset)}><b>{meta.label}</b><small>{meta.description}</small></button>; })}</div><p className="macro-preset-note">Доли считаются от калорий. Это ориентир, не медицинская рекомендация; любое БЖУ можно поправить вручную.</p></div>
...
function GoalEstimator({ person, onApply }: { person: Person; onApply: (estimate: GoalEstimate, kcal: number) => void }) {
  const [draft, setDraft] = useState<GoalEstimate>(person.estimate ?? { sex: "male", age: 30, height: 178, weight: 78, activity: "medium", goal: "lose" });
  const fields: { key: "age" | "height" | "weight"; label: string; unit: string; min: number; max: number }[] = [
    { key: "age", label: "Возраст", unit: "лет", min: 14, max: 90 },
    { key: "height", label: "Рост", unit: "см", min: 120, max: 230 },
    { key: "weight", label: "Вес", unit: "кг", min: 35, max: 250 },
  ];
  const complete = fields.every((field) => draft[field.key] >= field.min && draft[field.key] <= field.max);
  const kcal = complete ? estimateCalories(draft) : 0;
  const applied = complete && person.daily.kcal === kcal;
  const veryLow = complete && kcal <= 1300;
  return <div className="goal-estimator">
    <div className="estimator-sex" role="radiogroup" tabIndex={-1} onKeyDown={handleRadioKeyDown} aria-label="Пол">{(["male", "female"] as Sex[]).map((value) => <button key={value} role="radio" aria-checked={draft.sex === value} className={draft.sex === value ? "selected" : ""} onClick={() => setDraft({ ...draft, sex: value })}>{value === "male" ? "Мужчина" : "Женщина"}</button>)}</div>
    <div className="estimator-fields">{fields.map((field) => <label key={field.key}><span>{field.label}</span><input type="number" inputMode="numeric" min={field.min} max={field.max} value={draft[field.key] || ""} onChange={(event) => setDraft({ ...draft, [field.key]: Number(event.target.value) })} /><small>{field.unit}</small></label>)}</div>
    <label className="estimator-select"><span>Активность</span><select value={draft.activity} onChange={(event) => setDraft({ ...draft, activity: event.target.value as ActivityKey })}>{(Object.keys(activityMeta) as ActivityKey[]).map((key) => <option key={key} value={key}>{activityMeta[key].label}</option>)}</select></label>
    <label className="estimator-select"><span>Цель</span><select value={draft.goal} onChange={(event) => setDraft({ ...draft, goal: event.target.value as GoalKey })}>{(Object.keys(goalMeta) as GoalKey[]).map((key) => <option key={key} value={key}>{goalMeta[key].label}</option>)}</select></label>
    <div className="estimator-result"><p>{complete ? <><b>≈ {kcal} ккал в день</b><small>Формула Миффлина — Сан Жеора с коэффициентом активности. Ориентир, а не медицинская рекомендация: цифры можно поправить вручную.</small></> : <><b>Заполните параметры</b><small>Возраст {fields[0].min}–{fields[0].max}, рост {fields[1].min}–{fields[1].max} см, вес {fields[2].min}–{fields[2].max} кг.</small></>}</p><button className="secondary-button" disabled={!complete} onClick={() => onApply(draft, kcal)}>{applied ? "Применено ✓" : "Применить"}</button></div>
    {veryLow && <p className="inline-warning">Расчёт упирается в нижнюю границу 1200 ккал — проверьте параметры или выберите другую цель.</p>}
  </div>;
}
...
function CookingStep({ periodDays, cookEveryDays, remainder, decision, start, resolvedDays, canExtend, onDays, onDecision }: { periodDays: number; cookEveryDays: number; remainder: number; decision: "separate" | "extend" | "shorten" | null; start: string; resolvedDays: number; canExtend: boolean; onDays: (days: number) => void; onDecision: (value: "separate" | "extend" | "shorten") => void }) {
  ...
  return <><StepIntro icon="pot" kicker="Ритм готовки" title="На сколько дней готовим за раз?" text="Выберите размер одной партии. Мы учтём хранение и заморозку." /><section className="glass-card cooking-card"><div className="day-scale" role="radiogroup" tabIndex={-1} onKeyDown={handleRadioKeyDown} aria-label="Размер одной партии">{[1, 2, 3, 4, 5, 6, 7].map((days) => <button key={days} role="radio" aria-checked={cookEveryDays === days} className={cookEveryDays === days ? "selected" : ""} onClick={() => onDays(days)}><b>{days}</b><small>{pluralRu(days, "день", "дня", "дней")}</small></button>)}</div><div className="batch-timeline">{blocks.map((batch) => <div key={batch.id}><span>{batch.index + 1}</span><p><b>Готовка {batch.index + 1}</b><small>{formatDate(batch.start)} — {formatDate(batch.end)} · {countRu(batch.days, "день", "дня", "дней")}</small></p></div>)}</div></section>{remainder > 0 && <div className="remainder-sheet glass-card" role="radiogroup" tabIndex={-1} onKeyDown={handleRadioKeyDown} aria-label="Как поступить с последним блоком"><p className="kicker">Нужно ваше решение</p><h3>{countRu(periodDays, "день", "дня", "дней")} не делятся на {cookEveryDays} без остатка</h3><p>Последний блок — {countRu(remainder, "день", "дня", "дней")}. Как поступить?</p><button role="radio" aria-checked={decision === "separate"} className={decision === "separate" ? "selected" : ""} onClick={() => onDecision("separate")}><span><Icon name="pot" /></span><div><b>Приготовить остаток отдельно</b><small>Оставить даты, финальная мини-готовка на {countRu(remainder, "день", "дня", "дней")}</small></div><i>{decision === "separate" ? "✓" : ""}</i></button><button role="radio" aria-checked={decision === "extend"} className={decision === "extend" ? "selected" : ""} disabled={!canExtend} onClick={() => onDecision("extend")}><span><Icon name="plus" /></span><div><b>Добавить {countRu(cookEveryDays - remainder, "день", "дня", "дней")}</b><small>{canExtend ? `Новый конец: ${formatDate(addDays(start, periodDays + cookEveryDays - remainder - 1))}` : "Получится больше 14 дней — выберите другой вариант"}</small></div><i>{decision === "extend" ? "✓" : ""}</i></button><button role="radio" aria-checked={decision === "shorten"} className={decision === "shorten" ? "selected" : ""} disabled={periodDays - remainder < 1} onClick={() => onDecision("shorten")}><span><Icon name="minus" /></span><div><b>Убрать {countRu(remainder, "день", "дня", "дней")}</b><small>Новый конец: {formatDate(addDays(start, periodDays - remainder - 1))}</small></div><i>{decision === "shorten" ? "✓" : ""}</i></button></div>}</>;
}
...
function MenuStep({ position, positions, currentIndex, selections, style, people, onJump, onChoose, onRepeat }: { position: { batch: Batch; slot: MealSlot }; positions: { batch: Batch; slot: MealSlot }[]; currentIndex: number; selections: Record<string, string>; style: MenuStyle; people: Person[]; onJump: (index: number) => void; onChoose: (id: string) => void; onRepeat: (id: string, slot: MealSlot) => void }) {
  ...
  useEffect(() => { document.querySelector<HTMLElement>(".position-strip .current")?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "center" }); }, [currentIndex]);
  return <><StepIntro icon={mealMeta[position.slot].icon} kicker={`${completed} из ${positions.length} выбрано`} title={`${mealMeta[position.slot].label} · готовка ${position.batch.index + 1}`} text={`${formatDate(position.batch.start)} — ${formatDate(position.batch.end)}. Выберите один из пяти вариантов.`} /><div className="position-strip" aria-label="Позиции меню">{positions.map((item, index) => <button key={`${item.batch.id}-${item.slot}`} className={`${index === currentIndex ? "current" : ""} ${selections[selectionKey(item.batch, item.slot)] ? "done" : ""}`} onClick={() => onJump(index)} aria-current={index === currentIndex ? "step" : undefined} aria-label={`Готовка ${item.batch.index + 1}, ${mealMeta[item.slot].label}`}><span><Icon name={mealMeta[item.slot].icon} /></span><small>{item.batch.index + 1}</small></button>)}</div><div className="menu-candidates" role="radiogroup" tabIndex={-1} onKeyDown={handleRadioKeyDown} aria-label={`Варианты для ${mealMeta[position.slot].label.toLowerCase()}`}>{candidates.map((recipe) => { const selected = selectedId === recipe.id; const fit = fitScore(recipe, people, position.slot); return <article className={`candidate-card glass-card ${selected ? "selected" : ""}`} key={recipe.id}><button className="candidate-main" role="radio" aria-checked={selected} onClick={() => onChoose(recipe.id)}><RecipeArtwork recipe={recipe} className="candidate-art" badge={`${recipe.time} мин`} /><div className="candidate-copy"><div className="fit-badge">Совпадение {fit}%</div><h3>{recipe.title}</h3><p>{formatMacros(recipe.macros)}</p><small>{styleNote(recipe, style)} · хранится {countRu(recipe.storageDays, "день", "дня", "дней")}</small></div><i>{selected ? "✓" : ""}</i></button>{selected && positions.filter((item) => item.slot === position.slot).length > 1 && <button className="repeat-button" onClick={() => onRepeat(recipe.id, position.slot)}>Повторить во всех готовках</button>}</article>; })}</div></>;
}
...
function ReviewStep({ plan, onEdit }: { plan: ActivePlan; onEdit: (step: number) => void }) {
  ...
  return <><StepIntro icon="check" kicker="Почти готово" title="Проверьте план" text="После сохранения он появится в неделе, а продукты — в покупках." /><section className="review-hero glass-card"><div><p className="kicker">{formatDate(plan.start)} — {formatDate(plan.end)}</p><h2>{countRu(plan.periodDays, "день", "дня", "дней")} · {countRu(plan.people.length, "человек", "человека", "человек")}</h2></div><span><Icon name={styleMeta[plan.menuStyle].icon} /></span><div className="review-stats"><p><b>{plan.batches.length}</b><small>{pluralRu(plan.batches.length, "готовка", "готовки", "готовок")}</small></p><p><b>{recipeIds.size}</b><small>{pluralRu(recipeIds.size, "рецепт", "рецепта", "рецептов")}</small></p><p><b>{totalPortions}</b><small>{pluralRu(totalPortions, "порция", "порции", "порций")}</small></p><p><b>{plan.shopping.length}</b><small>{pluralRu(plan.shopping.length, "продукт", "продукта", "продуктов")}</small></p></div></section><section className="review-list glass-card"><button onClick={() => onEdit(0)}><span><Icon name="calendar" /></span><div><b>Период</b><small>{formatDate(plan.start)} — {formatDate(plan.end)}</small></div><i>Изменить</i></button><button onClick={() => onEdit(3)}><span><Icon name="person" /></span><div><b>Люди и КБЖУ</b><small>{plan.people.map((person) => person.name).join(", ")}</small></div><i>Изменить</i></button><button onClick={() => onEdit(4)}><span><Icon name="pot" /></span><div><b>График готовки</b><small>Каждые {countRu(plan.cookEveryDays, "день", "дня", "дней")} · {countRu(plan.batches.length, "блок", "блока", "блоков")}</small></div><i>Изменить</i></button><button onClick={() => onEdit(5)}><span><Icon name="sparkles" /></span><div><b>Выбранное меню</b><small>{countRu(Object.keys(plan.selections).length, "позиция", "позиции", "позиций")}</small></div><i>Изменить</i></button></section><section className="shopping-preview glass-card"><div className="group-title"><h3>Покупки</h3><span>{plan.shopping.length}</span></div>{plan.shopping.slice(0, 5).map((item) => <p key={item.key}><span>{item.name}</span><b>{item.quantity.toLocaleString("ru-RU")} {item.unit}</b></p>)}{plan.shopping.length > 5 && <small>и ещё {countRu(plan.shopping.length - 5, "продукт", "продукта", "продуктов")}</small>}</section></>;
}
...
function SuccessSheet({ plan, onOpen, onEdit }: { plan: ActivePlan; onOpen: (tab: Tab) => void; onEdit: () => void }) {
  ...
  return <Modal labelledBy={phase === "summary" ? "success-title" : "notifications-title"} className={phase === "notifications" ? "notification-modal" : ""} onClose={() => onOpen("week")}>{phase === "summary" ? <><div className="success-burst"><Icon name="check" /></div><p className="kicker">Всё получилось</p><h2 id="success-title">План готов!</h2><p>{countRu(plan.periodDays, "день", "дня", "дней")} · {countRu(plan.batches.length, "готовка", "готовки", "готовок")} · {countRu(new Set(Object.values(plan.selections)).size, "рецепт", "рецепта", "рецептов")} · {countRu(plan.shopping.length, "продукт", "продукта", "продуктов")}</p><InstallInline /><button className="primary-button" onClick={() => setPhase("notifications")}>Настроить напоминания <span>→</span></button><button className="secondary-button" onClick={() => onOpen("week")}>Открыть план без них</button><button className="text-button" onClick={onEdit}>Изменить план</button></> : <NotificationSetupPanel plan={notificationPlanFor(plan)} clientId={clientId()} deviceId={deviceId()} onDone={() => onOpen("week")} onCancel={() => onOpen("week")} />}</Modal>;
}
...
function RecipeView({ context, onBack, onChangePlan }: { context: RecipeContext; onBack: () => void; onChangePlan?: (plan: ActivePlan) => Promise<void> }) {
  ...
  const effortSummary = `${countRu(recipe.effort.knifeActions, "нарезка", "нарезки", "нарезок")} · ${countRu(recipe.effort.cookware, "единица посуды", "единицы посуды", "единиц посуды")} · ${countRu(recipe.effort.activeActions, "действие", "действия", "действий")}`;
  const ingredientScope = batch ? `На ${countRu(batch.days, "день", "дня", "дней")} · ${countRu(eaters.length, "человек", "человека", "человек")}` : "На одну базовую порцию";
  const storageHeading = recipe.storage.ambient ? `Сухое хранение: ${countRu(recipe.storageDays, "день", "дня", "дней")}` : recipe.freezable ? `Холодильник: ${countRu(recipe.storageDays, "день", "дня", "дней")} или заморозка` : `Только холодильник: ${countRu(recipe.storageDays, "день", "дня", "дней")}`;
  const storageSummary = freezeDays > 0 ? `Оставьте на ${countRu(recipe.storageDays, "день", "дня", "дней")} в холодильнике, ещё ${countRu(freezeDays, "порцию", "порции", "порций")} каждого человека заморозьте.` : recipe.storage.ambient ?? `Ориентир для холодильника: ${countRu(recipe.storageDays, "день", "дня", "дней")}.`;
  ...
  return <main className="app-shell recipe-detail"><div className="ambient ambient-one" /><header className="detail-header"><button className="icon-button glass" onClick={() => { history.back(); window.setTimeout(() => backRef.current(), 250); }} aria-label="Назад"><Icon name="chevron" className="back-chevron" /></button><span className="glass">{recipe.effort.activeMinutes} мин активно · {recipe.time} всего</span></header><section className="detail-hero"><RecipeArtwork recipe={recipe} className="detail-food glass" sourcePhoto /><p className="kicker">{mealMeta[recipe.slot].label} · {originLabel}</p><h1>{recipe.title}</h1><div className="detail-macros">{(["kcal", "protein", "fat", "carbs"] as MacroKey[]).map((key) => <span key={key}><b>{displayMacros[key]}</b><small>{macroLabels[key]}{key === "kcal" ? "кал" : ""}</small></span>)}</div></section><section className="macro-tuner glass-card"><div className="tuner-heading"><div><p className="kicker">Гибкая порция</p><h2>Подстройка КБЖУ</h2></div>{person && <select aria-label="Для кого настроить порцию" value={person.id} onChange={(event) => selectPerson(event.target.value)}>{eaters.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select>}</div><p className="tuner-copy">{person ? "База уже подогнана под цель. Здесь можно докрутить состав порции в разумных пределах." : "Попробуйте базовую порцию. В плане Mise начнёт с цели каждого человека."}</p><div className="tuner-controls">{([{ key: "protein", label: "Белковая часть", value: displayMacros.protein, range: recipe.flex.protein }, { key: "carbs", label: "Гарнир", value: displayMacros.carbs, range: recipe.flex.carbs }, { key: "fat", label: "Жиры и соус", value: displayMacros.fat, range: recipe.flex.fat }] as const).map((control) => <label key={control.key} aria-label={control.label}><span><b>{control.label}</b><em>{Math.round(draft[control.key] * 100)}% · {control.value} г</em></span><input type="range" min={control.range[0]} max={control.range[1]} step="0.05" value={draft[control.key]} onChange={(event) => updateDraft(control.key, Number(event.target.value))} /></label>)}</div><div className="tuner-actions"><button className="secondary-button" onClick={() => { setDraft(automaticTuning); setSaveStatus("idle"); }}>{person ? "Вернуть к цели" : "Сбросить"}</button>{person && <button className="primary-button" disabled={saveStatus === "saving"} onClick={saveTuning}>{saveStatus === "saving" ? "Сохраняем…" : saveStatus === "saved" ? "Сохранено ✓" : "Сохранить и пересчитать"}</button>}</div>{saveStatus === "saved" && <p className="tuner-saved" role="status">Порция сохранена — список покупок пересчитан.</p>}{saveStatus === "error" && <p className="tuner-error" role="alert">Не удалось сохранить. Изменения не попали в план.</p>}</section><section className="recipe-info-grid"><article className="glass-card"><span><Icon name="sparkles" /></span><div><b>{recipe.effort.level === "low" ? "Низкая сложность" : "Высокая сложность"}</b><small>{effortSummary}</small></div></article><article className="glass-card"><span><Icon name="clock" /></span><div><b>{recipe.effort.activeMinutes} мин активно</b><small>{recipe.time} мин общего времени</small></div></article></section><section className="detail-panel glass-card"><div className="detail-tabs"><button className={section === "ingredients" ? "selected" : ""} onClick={() => setSection("ingredients")}>Ингредиенты</button><button className={section === "steps" ? "selected" : ""} onClick={() => setSection("steps")}>Готовить</button><button className={section === "portion" ? "selected" : ""} onClick={() => setSection("portion")}>Разложить</button></div>{section === "ingredients" && <div className="detail-list"><div className="detail-note"><span><Icon name="info" /></span><p><b>{ingredientScope}</b><small>Количество меняется вместе с рычагами КБЖУ</small></p></div>{recipe.ingredients.map((ingredient) => { const totalScale = totalIngredientScale(ingredient); return <div className="ingredient-row" key={ingredient.id}><span>✓</span><p>{ingredient.name}<small>{ingredient.group}</small></p><b>{ingredient.unit === "шт." ? round(ingredient.quantity * totalScale, 1) : round(ingredient.quantity * totalScale / 5) * 5} {ingredient.unit}</b></div>; })}</div>}{section === "steps" && <ol className="cooking-steps">{recipe.steps.map((text, index) => <li key={`${text}-${index}`}><span>{index + 1}</span><p>{text}</p></li>)}</ol>}{section === "portion" && <div className="portion-section">{batch && slot && plan ? <><div className="detail-note"><span><Icon name="container" /></span><p><b>{countRu(batch.days, "контейнер", "контейнера", "контейнеров")} на человека</b><small>Подпишите имя, приём пищи и даты</small></p></div>{eaters.map((eater, index) => { const portion = portionFor(eater, slot, recipe, eater.id === person?.id ? draft : plan.tuning?.[tuningKey(batch, slot, eater)]); return <article className="portion-card" key={eater.id}><div className={`person-dot tone-${index}`}>{eater.name.slice(0, 1)}</div><div><h3>{eater.name}</h3><p><b>{batch.days} × примерно {portion.grams} г</b></p><small>{formatMacros(portion.actual)} на контейнер</small><em>Подпись: {eater.name} / {mealMeta[slot].label.toLowerCase()} / {formatDate(batch.start)}–{formatDate(batch.end)}</em></div></article>; })}<section className="storage-card"><span>{freezeDays > 0 ? <Icon name="snowflake" /> : <Icon name="check" />}</span><div><h3>{freezeDays > 0 ? "Часть порций заморозить" : recipe.storage.ambient ? "Хранить в сухой банке" : "Хранить в холодильнике"}</h3><p>{storageSummary}</p></div></section></> : <section className="detail-note"><span><Icon name="target" /></span><p><b>Точная раскладка появится в плане</b><small>Мы учтём КБЖУ и цели каждого человека.</small></p></section>}</div>}</section><RecipePackingGuide recipe={recipe} /><section className="recipe-storage glass-card"><p className="kicker">Ориентиры хранения</p><h2>{storageHeading}</h2><p>{recipe.storage.ambient ?? recipe.storage.refrigerator}</p>{recipe.storage.freezer && <p><b>Морозилка{recipe.storage.freezerDays ? ` — до ${countRu(recipe.storage.freezerDays, "дня", "дней", "дней")}` : ""}:</b> {recipe.storage.freezer}</p>}{recipe.storage.freezeParts && <p><b>Что замораживать:</b> {recipe.storage.freezeParts}</p>}{recipe.storage.thaw && <p><b>Как разморозить:</b> {recipe.storage.thaw}</p>}<small>Сроки — консервативные ориентиры, а не гарантия.</small></section><section className="recipe-source glass-card"><p className="kicker">Происхождение</p><h2>{originLabel}</h2>{recipe.provenance.kind === "parsed" ? <><a href={recipe.provenance.sourceUrl} target="_blank" rel="noreferrer">{recipe.provenance.sourceTitle} ↗</a>{recipe.provenance.adaptation && <p>Адаптация для Mise: {recipe.provenance.adaptation}</p>}<small>Найдено по запросу «{recipe.provenance.sourceQuery}».</small></> : <><p>Рецепт собран для курированного каталога Mise.</p><small>{recipe.provenance.basedOn?.length ? `Опирается на ${countRu(recipe.provenance.basedOn.length, "отобранный источник", "отобранных источника", "отобранных источников")}.` : "Без внешнего рецепта-прототипа."}</small></>}</section></main>;
}
