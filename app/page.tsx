...
function WeekScreen({ plan, loading, loadError, onBuild, onRepeat, onEditMenu, onOpenRecipe }: { plan: ActivePlan | null; loading: boolean; loadError: boolean; onBuild: () => void; onRepeat: () => void; onEditMenu: (batchId: string) => void; onOpenRecipe: (context: RecipeContext) => void }) {
  ...
  return <section className="screen week-screen">
    ...
    <button className="prep-callout glass-card" onClick={() => setSelectedDate(batch.start)} aria-label={`Открыть день готовки ${batch.index + 1}`}><span className="prep-icon">♨</span><div><p className="kicker">Готовка {batch.index + 1}</p><h3>{formatDate(batch.start)} — {formatDate(batch.end)}</h3><p>{batch.days} {batch.days === 1 ? "день" : batch.days < 5 ? "дня" : "дней"} · {dayMeals.length} блюд</p></div><span className="soft-chevron">›</span></button>
...
function ShoppingScreen({ plan, onBuild, onChange }: { plan: ActivePlan | null; onBuild: () => void; onChange: (plan: ActivePlan) => void }) {
  ...
  function toggle(key: string) { onChange({ ...currentPlan, shopping: currentPlan.shopping.map((item) => item.key === key ? { ...item, checked: !item.checked } : item) }); }
