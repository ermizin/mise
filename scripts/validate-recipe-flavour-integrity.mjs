...
const DEFAULT_RELEASE_AUDIT = "data/recipe-release-audit.json";

const normalized = (value) => String(value ?? "").normalize("NFKC").toLocaleLowerCase("ru-RU");

export function validateRecipeFlavourIntegrity({ catalog, registry, releaseAudit }) {
  ...
  const releaseCards = new Map((releaseAudit?.cards ?? []).map((card) => [card.id, card]));
  const violations = [];
  for (const expectation of registry.recipes) {
    if (releaseCards.get(expectation.id)?.verdict === "blocked") continue;
...
export async function loadRecipeFlavourIntegrityInputs({ cwd = process.cwd() } = {}) {
  const [registry, catalog, releaseAudit] = await Promise.all([
    readFile(resolve(cwd, DEFAULT_REGISTRY), "utf8").then(JSON.parse),
    readFile(resolve(cwd, DEFAULT_CATALOG), "utf8").then(JSON.parse),
    readFile(resolve(cwd, DEFAULT_RELEASE_AUDIT), "utf8").then(JSON.parse),
  ]);
  return { registry, catalog, releaseAudit };
}
