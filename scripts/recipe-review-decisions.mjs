...
export const RESOLUTION_VERDICTS = Object.freeze(["ready", "review_required", "blocked", "excluded"]);
...
function emptyResolutions() {
  return { schemaVersion: 1, sourceExport: {}, exclusions: [], resolvedReasons: [], decisions: [] };
}
...
export function validateRecipeReviewResolutions(value, cards) {
  ...
  for (const key of ["sourceExport", "exclusions", "resolvedReasons", "decisions"]) {
    if (!(key in value)) throw new Error(`Recipe review resolutions: missing ${key}`);
  }
  if (!isObject(value.sourceExport) || !Array.isArray(value.exclusions) || !Array.isArray(value.resolvedReasons) || !Array.isArray(value.decisions)) {
    ...
    const verdict = excluded.has(card.id)
      ...
      : reasons.some((reason) => reason.severity === "blocked")
        ...
        : reasons.some((reason) => reason.severity === "review_required")
          ? "review_required"
          : "ready";
    return { ...card, verdict, reasons, resolvedReasons, exclusion: excluded.get(card.id) ?? null };
  });
