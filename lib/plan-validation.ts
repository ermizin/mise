...
function validForBatch(recipe: RegistryRecipe, slot: string, days: number) {
  return recipeSupportsSlot(recipe, slot) && (recipe.storageDays >= days || recipe.freezable);
}
...
export function validatePlanForPersistence(value: unknown): PlanValidationResult {
  ...
  for (const batch of orderedBatches) for (const slot of planMealSlots) {
    ...
    if (!selected || !validForBatch(selected, slot, batch.days)) return { valid: false, error: "plan references an unavailable recipe", status: 422 };
    ...
    if (!hasExplicitAssignments) {
      ...
      for (const assignment of raw) {
        ...
        if (!recipe || !validForBatch(recipe, slot, batch.days)) return { valid: false, error: "plan references an unavailable recipe", status: 422 };
