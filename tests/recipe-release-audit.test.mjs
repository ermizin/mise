...
test("release audit joins editorial and nutrition gates for every card", () => {
  ...
  assert.equal(report.counts.ready + report.counts.review_required + report.counts.blocked + report.counts.excluded, report.total);
  for (const card of report.cards) {
    assert.ok(["ready", "review_required", "blocked", "excluded"].includes(card.verdict));
