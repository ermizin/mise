...
async function core() {
  ...
  for (const name of ["validate", "schedule"]) {
    ...
    vm.runInNewContext(ts.transpileModule(await readFile(url, "utf8"), { compilerOptions: { target: ts.ScriptTarget.ES2022, module: ts.ModuleKind.CommonJS } }).outputText, { module, exports: module.exports, require: (id) => modules[id] ?? createRequire(url)(id), Map, Set, Math, Object, Array, JSON, Number, Infinity }, { filename: url.pathname });
...
test("one pot stays leased through unload while the cook preps the second dish during heating", async () => {
  ...
  const aStart = op("a-start", "start_heat", { duration: 10, resources: [use("cook", "cook"), use("pot", "pot"), use("burner", "burner")], holds: [ { resourceId: "pot", kind: "pot", releaseAfterOpId: "a-unload" }, { resourceId: "burner", kind: "burner", releaseAfterOpId: "a-unload" } ] });
  const aHeat = op("a-heat", "heat", { deps: ["a-start"], duration: 100, background: true, resources: [use("pot", "pot"), use("burner", "burner")] });
  const aCheck = op("a-check", "intervention", { deps: ["a-heat"], duration: 10, resources: [use("cook", "cook"), use("pot", "pot"), use("burner", "burner")] });
  const aUnload = op("a-unload", "unload", { deps: ["a-check"], duration: 10, resources: [use("cook", "cook"), use("pot", "pot")] });
  const bPrep = op("b-prep", "prep", { duration: 30, resources: [use("cook", "cook"), use("board", "board")] });
  const bStart = op("b-start", "start_heat", { deps: ["b-prep"], duration: 10, resources: [use("cook", "cook"), use("pot", "pot"), use("burner", "burner")] });
...
test("an oven resource is exclusive even when two heating chains would otherwise run in parallel", async () => {
  ...
  const chain = (prefix) => [op(`${prefix}-start`, "start_heat", { duration: 5, resources: [use("cook", "cook"), use("oven", "oven"), use(`${prefix}-tray`, "tray")] }), op(`${prefix}-heat`, "heat", { deps: [`${prefix}-start`], duration: 40, background: true, resources: [use("oven", "oven"), use(`${prefix}-tray`, "tray")] }), op(`${prefix}-check`, "intervention", { deps: [`${prefix}-heat`], duration: 5, resources: [use("cook", "cook"), use("oven", "oven"), use(`${prefix}-tray`, "tray")] })];
...
test("two mandatory checks are reserved at different future cook times", async () => {
  const { scheduleCookingSession, validateCookingSchedule } = await core();
  const chain = (prefix, oven) => [op(`${prefix}-start`, "start_heat", { duration: 5, resources: [use("cook", "cook"), use(oven, "oven")] }), op(`${prefix}-heat`, "heat", { deps: [`${prefix}-start`], duration: 30, background: true, resources: [use(oven, "oven")] }), op(`${prefix}-check`, "intervention", { deps: [`${prefix}-heat`], duration: 8, resources: [use("cook", "cook"), use(oven, "oven")] })];
...
test("background waiting leaves the cook free and source durations are unchanged at comfortable pace", async () => {
  const { scheduleCookingSession } = await core();
  const start = op("a-start", "start_heat", { duration: 10, resources: [use("cook", "cook"), use("oven", "oven")] });
  const heat = op("a-heat", "heat", { deps: ["a-start"], duration: 100, background: true, resources: [use("oven", "oven")] });
...
test("validator rejects malformed graphs and never lets a dropped operation pass", async () => {
  ...
  const bad = op("bad", "prep", { duration: Number.NaN, resources: [use("cook", "cook"), use("ghost", "pot")] });
  const cycle = op("cycle", "portion", { deps: ["cycle"], resources: [use("cook", "cook")] });
...
test("tiny exhaustive oracle confirms the best of both oven-chain orders", async () => {
  const { scheduleCookingSession } = await core();
  const chain = (prefix, heatDuration) => [op(`${prefix}-start`, "start_heat", { duration: 2, resources: [use("cook", "cook"), use("oven", "oven")] }), op(`${prefix}-heat`, "heat", { deps: [`${prefix}-start`], duration: heatDuration, background: true, resources: [use("oven", "oven")] }), op(`${prefix}-check`, "intervention", { deps: [`${prefix}-heat`], duration: 3, resources: [use("cook", "cook"), use("oven", "oven")] })];
